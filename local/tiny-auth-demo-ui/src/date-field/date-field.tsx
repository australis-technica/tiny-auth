...
export type DateFieldProps = Exclude<TextFieldProps, "value" | "onChange"> & { dateValue: Date | number, onDateChange(date: Date): any };
...
const DateField: StatelessComponent<DateFieldProps> = (props) => {
    const { dateValue, value, type, onChange, InputLabelProps, ...rest } = props;
    const handle: ChangeEventHandler<HTMLInputElement> = (e) => {
        props.onDateChange(toDate(e.target.value));
    }
    return <TextField
        type="date"
