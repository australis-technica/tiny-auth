 ...
 * 
 */
const getProfile: (findById: (id: string) => Promise<any>) => RequestHandler = (findById) => async (req, res, next) => {
  try {
    const result = await findById(req.user.id).then(withoutPassword);
